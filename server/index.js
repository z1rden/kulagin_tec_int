...
app.post('/tasklists/:tasklistId/tasks', async (request, response) => {
  const { taskName, nomer } = request.body;
  ...
  tasklists[tasklistId].tasks.push([taskName, nomer]);
  await writeData(tasklists);
  response
    .setHeader('Content-Type', 'application/json')
    .status(200)
...
app.put('/tasklists/:tasklistId/tasks/:taskId', async (request, response) => {
  const { newTaskName, newNomer } = request.body;
  const tasklistId = Number(request.params.tasklistId);
  ...
  tasklists[tasklistId].tasks[taskId] = [newTaskName, newNomer];
  await writeData(tasklists);
  response
    .setHeader('Content-Type', 'application/json')
    .status(200)
